...
def get_position(side, color = (255, 255, 255)):
    '''
    ...
    elif color == YELLOW pr color ==  LIGHT_YELLOW:
...
def get_point(map_grid, direction, color = (255, 255, 255)):
    '''
    ...
    if direction == 'left':
        side = map_grid[:, 0, :]
        x_position = get_position(side, color)
        ...
        side = map_grid[0, :, :]
        y_position = get_position(side, color)
...
def get_all_enter_exit_point(map_grids, enter_direction, exit_direction, downsample=1.0, color = (255, 255, 255)):
    '''
    ...
    enter_points = get_point(map_grids, enter_direction, color)
    exit_points = get_point(map_grids, exit_direction, color)
    enter_points, exit_points = transform(enter_points, exit_points, w, h)
    return enter_points, exit_points


def get_random_enter_exit_point(map_grid, enter_direction, exit_direction, downsample=1.0, last_exit_point=None, color = (255, 255, 255)):
    '''
    return the enter point & exit point
    '''
    enter_points, exit_points = get_all_enter_exit_point(
        map_grid, enter_direction, exit_direction, downsample, color)
...
if __name__ == '__main__':
    ...
    enter_point, exit_point = get_random_enter_exit_point(
        map_grids['1_2'], 'left', 'down', downsample=0.2, color = (255, 255, 255))
