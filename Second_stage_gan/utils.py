...
def cal_waysloss(ref, pred, pred_inv, convsize, std, device):
    '''
    calculate a loss to evaluate how far the coordinate are from the roadnet

    :param ref: (b,1,encode_size,encode_size) feature maps generated by encoder
    :param pred/pred_inv: (b,max_len,2) a sequence of coordinates
    :param convsize: (1) a int python scalar to indicate how many pixels(convsize*convsize) surrounding the coordinates will be taken into consideration
    '''
    pred = torch.clamp(pred,-1.,1.)
    pred_inv = torch.clamp(pred_inv,-1.,1.)

    encode_size = ref.shape[1]
    pad = int((convsize - 1) / 2)

    ref = F.pad(ref,(pad,pad,pad,pad),'reflect') # padded (b,1,e + (convsize-1) , .)
    guass_kernel = torch.tensor(gkern(convsize, std), dtype=torch.float).to(device)
    guass_kernel = guass_kernel.reshape(1,1,convsize,convsize) # (1,1,convsize,convsize)

    ref_guass = F.conv2d(ref, guass_kernel, padding=pad) # (b,1,encode_size,encode_size)

    loss = F.grid_sample(ref_guass, pred.unsqueeze(1)).mean() + F.grid_sample(ref_guass, pred_inv.unsqueeze(1)).mean()

    return loss


def gkern(kernlen=7, std=1):
...
def save_checkpoint(save_path, epoch, epochs_since_improvement, encoder, decoder, encoder_optimizer,
    ...
    filename = save_path +'checkpoint_latest.pth'
...
def save_checkpoint_gan(save_path, epoch, epochs_since_G_improvement, epochs_since_D_improvement, encoder, decoder, D, encoder_optimizer,
