...
class Road(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        name = item["name"]
        #print(os.path.join(self.root, 'road', item["img"]))
        image = cv2.imread(os.path.join(self.root, 'road', item["img"]),
                           cv2.IMREAD_COLOR)
        size = image.shape
        ...
        label = cv2.imread(os.path.join(self.root, 'road', item["label"]),
