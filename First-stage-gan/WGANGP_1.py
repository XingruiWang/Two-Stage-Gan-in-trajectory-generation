...
def train(dataloader, optimizerD, optimizerG, netD, netG):
    for epoch in range(opt.start_iter,opt.niter):
        for i, (real_imgs) in enumerate(dataloader):
            ...
            if epoch % 1 == 0:
                vutils.save_image((real_imgs/2+0.5)*255,
                                f'{opt.outf}/real_samples.png',
                                normalize=True)
                vutils.save_image((real_imgs/2+0.5)*255,f'{opt.outf}/real_samples_false.png',normalize=True)
                vutils.save_image((netG(noise).detach()/2+0.5)*255,
                                f'{opt.outf}/fake_samples_epoch_{epoch}.png',
                                normalize=True)
                vutils.save_image((netG(noise).detach()/2+0.5)*255,f'{opt.outf}/fake_samples_epoch_{epoch}_false.png',normalize=False)
