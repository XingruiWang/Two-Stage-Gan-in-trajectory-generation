    ...
    for epoch in range(opt.start_iter,opt.niter):
        for i, (real_imgs) in enumerate(dataloader):
            ...
            if i % 100 == 0:
                ...
                writer.add_scalar('log/loss_D', errD.item(),time_step )
                writer.add_scalar('log/loss_G', errG.item(), time_step)
                writer.add_scalar('log/gradients_penalty', gradient_penalty.item(), time_step)
            if epoch % 1 == 0:
