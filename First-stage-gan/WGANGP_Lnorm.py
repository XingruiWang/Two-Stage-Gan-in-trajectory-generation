...
class Generator(nn.Module):
    def __init__(self):
        ...
        self.LayerNorm = _get_norm_layer_2d('layer_norm')
        self.main = nn.Sequential(
        # inputs is Z, going into a convolution
        nn.ConvTranspose2d(nz, ngf * 4, 4, 1, 0, bias=False),
        self.LayerNorm(ngf * 4),
        nn.ReLU(True),
        # state size. (ngf*8) x 4 x 4
        nn.ConvTranspose2d(ngf * 4, ngf * 2, 4, 2, 1, bias=False),
        self.LayerNorm(ngf * 2),
        nn.ReLU(True),
...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.LayerNorm(ndf * 2),
        nn.LeakyReLU(0.2, inplace=True),

        # state size. (ndf*4) x 8 x 8
        nn.Conv2d(ndf * 2, ndf * 4, 4, 2, 1, bias=False),
        self.LayerNorm(ndf * 4),
        nn.LeakyReLU(0.2, inplace=True),
    ...
    for epoch in range(opt.start_iter,opt.niter):
        for i, (real_imgs) in enumerate(dataloader):
            ...
            if i % 100 == 0:
                ...
                writer.add_scalar('log_lnorm/loss_D', errD.item(),time_step )
                writer.add_scalar('log_lnorm/loss_G', errG.item(), time_step)
                writer.add_scalar('log_lnorm/gradients_penalty', gradient_penalty.item(), time_step)
            if epoch % 1 == 0:
